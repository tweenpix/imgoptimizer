...
echo "режим работы скрипта с 22 до 6 утра"
	...
	if [[ $(which identify) && $(which mogrify) && $(which svgo) && $(which squoosh-cli) ]]; then
		...
		#mapfile -d $'\0' allpics < <(find . -maxdepth 1 \( -iname '*.jpeg' -o -iname '*.jpg' -o -iname '*.png' \) -type f -print0)
		#echo ${#allpics[@]}; return number files
...
exit 1
				...
				if [ ! -f $current.avif ]; then
					echo -e "${LYELLOW}need create ${DBOLD} avif from $current"
					tput sgr0
		...
		for cf in $(find . -maxdepth 1 \( -iname '*.jpeg' -o -iname '*.jpg' -o -iname '*.png' \) -type f); do
			...
			if [[ $(stat -c%s "$cf.webp") -gt $(stat -c%s "$cf") ]]; then
				...
				echo -e "webp is ${LYELLOW}bigger ${DBOLD}then $cf, need delete webp"
				tput sgr0
				rm "$cf.webp"
			fi
			...
			if [[ $(stat -c%s "$cf.avif") -gt $(stat -c%s "$cf") ]]; then
				...
				# echo "$cf is " && stat -c%s "$cf"
				echo -e "avif is ${LYELLOW}bigger ${DBOLD}then $cf, need delete avif"
				tput sgr0
				rm "$cf.avif"
			fi
		done
