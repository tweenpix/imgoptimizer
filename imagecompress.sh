...
find . -maxdepth 1 -iname '*bmp' -type f -exec mogrify -format jpg {} \;

#svgo
find . -maxdepth 1 -iname '*svg' -type f -exec svgo {} \;
...
for current in $(find . -maxdepth 1 \( -iname '*.jpeg' -o -iname '*.jpg' -o -iname '*.png' \) -type f); do
	if [ ! -f $current.webp ]; then
		...
		squoosh-cli --webp '{"quality":75, "method":4,"sns_strength":50,"filter_strength":60,"filter_type":1,"segments":4,"pass":1,"show_compressed":0,"preprocessing":0, "autofilter":0, "partition_limit":0, "alpha_compression":1, "alpha_filtering":1, "alpha_quality":100,"lossless":0}' $current.webp
	fi
